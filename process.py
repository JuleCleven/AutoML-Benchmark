...
#              'crit_data_types','crit_data_types', 'crit_data_types', 'crit_input', 'crit_processing', 'crit_processing',
...
#              'crit_ml_algorithms', 'crit_ml_algorithms', 'crit_ml_algorithms', 'crit_ml_algorithms',
#              'crit_ml_algorithms', 'crit_ml_algorithms', 'crit_ml_algorithms', 'crit_ml_algorithms',
#              'crit_ml_algorithms', 'crit_anti_overfitting_approach', 'crit_evaluation_approach', 'crit_search', 'crit_gui',
#              'crit_hyper_selection', 'crit_metrics', 'crit_charts', 'crit_web_service','crit_online_model_training',
...
def process():
    ...
    for system in systems:
        ...
        columns = [system.payment, system.programming_skills, system.datascience_skills, system.python, system.r,
                   ...
                   system.feature_generation, system.integrated_modeling, system.training, system.decision_tree,
                   system.gaussian_processes, system.annealing, system.tree_of_parzen_estimators,
                   system.light_gbm, system.random_search, system.adaboost, system.gradient_boosting,
                   system.knn, system.random_forest, system.svm, system.xgboost, system.neural_network,
                   system.regression, system.xgboost_gb, system.regularized_greedy_forest,
    ...
    for system in systems:
        ...
        for k in range(len(request_keys)):
            if crit_info[crit_keys[k]] is True and columns[k] is not None and user_info[request_keys[k]] != columns[k]:
                crit[system] = True

    print(System.query.filter(System.name == 'User').all())
    results = []
    for s in sorted(score, key=score.get, reverse=True):
        if crit[s] is not True:
            results.append(s)
